...
function addCols( $tab )
{
    ...
    //while ( list( $lib, $pos ) = array(key($tab), current($tab)))
    {
...
function addLineFormat( $tab )
{
    ...
    //while ( list( $lib, $pos ) = next ($colonnes) )
    {
        if ( isset( $tab["$lib"] ) )
            $format[ $lib ] = $tab["$lib"];
    }
...
function addCadreTVAs($letras)
{
    ...
    $this->SetFont( "Arial", "", 10);
    $this->SetXY( $r1+1, $y2 - 13 );
    //$this->Cell(6,0, $letras);
    $this->MultiCell(120,4, $letras , 0, "L" , false);
    // Salto de línea
    $this->Ln();
}
...
function temporaire( $texte )
{
    ...
    $this->Rotate(50,80,190);
    $this->Text(0,200,$texte);
    $this->Rotate(0);
