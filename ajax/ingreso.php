...
switch ($_GET["op"]) {
	...
	case 'listarDetalle':
		...
		while ($reg=$rspta->fetch_object()) {
			...
			<td>'.$reg->nombre.'</td>
			<td>'.$reg->cantidad.'</td>
			<td>$'.number_format($reg->precio_compra).'</td>
			<td>$'.number_format($reg->precio_venta).'</td>
			<td>$'.number_format($reg->precio_compra*$reg->cantidad).'</td>
			<td></td>
			</tr>';
		...
		echo '<tfoot>
         ...
         <th></th>
         <th></th>
         <th><h4 id="total">$ '.number_format($total).'</h4><input type="hidden" name="total_compra" id="total_compra"></th>
       </tfoot>';
		break;
