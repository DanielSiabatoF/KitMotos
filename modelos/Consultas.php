...
public function totalventahoy(){
	$sql="SELECT IFNULL(SUM(total_venta),0) as total_venta FROM venta WHERE DATE(fecha_fin)=curdate()";
	return ejecutarConsulta($sql);
}
...
public function ventasultimos_12meses(){
	$sql=" SELECT DATE_FORMAT(fecha_fin,'%M') AS fecha, SUM(total_venta) AS total FROM venta GROUP BY MONTH(fecha_fin) ORDER BY fecha_fin DESC LIMIT 0,12";
	return ejecutarConsulta($sql);
}
